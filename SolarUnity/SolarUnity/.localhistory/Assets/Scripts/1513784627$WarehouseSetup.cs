            ...
            for (int i = 0; i < boxes.Count; i++)
            {
                ...
                switch (type)
                {
                    ...
                    case BoxTypes.Orderbox:
                        ...
                        box.transform.GetChild(2).GetComponent<TextMesh>().text = (boxes[i] as OrderBox)?.LineIsPickedStatuses.Select(kvp => kvp.Key).Count(l => l.Article.AreaCode == CurrentArea)
